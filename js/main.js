        ...
        /* Hero Canvas — animated grid with mouse repulsion */
        ...
        function initNodes() {
            nodes = [];
            const cols = Math.floor(W / 90) + 1, rows = Math.floor(H / 90) + 1;
            for (let r = 0; r < rows; r++)for (let c = 0; c < cols; c++) {
                nodes.push({ ox: c * 90 + 45, oy: r * 90 + 45, x: c * 90 + 45, y: r * 90 + 45, vx: 0, vy: 0, });
            }
        }
        ...
        function drawCanvas() {
            ...
            nodes.forEach(n => {
                ...
                if (dist < 180) { const f = (180 - dist) / 180 * 18; n.vx += dx / dist * f; n.vy += dy / dist * f; }
                n.vx += (n.ox - n.x) * 0.04; n.vy += (n.oy - n.y) * 0.04;
                n.vx *= 0.82; n.vy *= 0.82;
                n.x += n.vx; n.y += n.vy;
                /* floating drift */
                const wave = Math.sin(t + n.ox * 0.01 + n.oy * 0.008) * 3;
            ...
            nodes.forEach((n, i) => {
                nodes.slice(i + 1).forEach(m => {
                    ...
                    if (d < 110) {
                        ...
                        ctx.strokeStyle = `rgba(10,95,255,${a})`; ctx.lineWidth = 0.6; ctx.stroke();
                    }
                });
                /* dot */
                ...
                ctx.fillStyle = `rgba(${glow > 0.3 ? '0,200,150' : '10,95,255'},${0.12 + glow * 0.3})`; ctx.fill();
            });
        ...
        reveals.forEach((el, i) => { el.dataset.delay = 0; ro.observe(el); });

        /* stagger grid children */
        function staggerGrid(sel, delay = 80) {
        ...
        staggerGrid('.lead-grid .lead-card', 80);
        staggerGrid('.cs-list .cs-item', 80);
        staggerGrid('.pres-cities .pres-city', 70);

        /* Process line animation */
        const procLine = document.getElementById('procLine');
        if (procLine) {
            const procObs = new IntersectionObserver(entries => {
                if (entries[0].isIntersecting) procLine.style.width = '80%';
            }, { threshold: 0.3 });
            const procSection = document.querySelector('.process');
            if (procSection) procObs.observe(procSection);
        }

        /* Investor metric bars */
        document.querySelectorAll('.im-bar').forEach(bar => {
            const target = bar.style.width; bar.style.width = '0%';
            const obs = new IntersectionObserver(entries => {
                if (entries[0].isIntersecting) { bar.style.width = target; obs.disconnect(); }
            }, { threshold: 0.5 });
        ...
        window.addEventListener('scroll', () => {
            ...
            if (heroPhoto) heroPhoto.style.transform = `scale(1.06) translateY(${sy * 0.15}px)`;
        }, { passive: true });
